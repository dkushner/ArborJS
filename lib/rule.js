...
var Rule = function () {
  ...
  function Rule(predecessor, production) {
    ...
    if (_lodash2.default.includes(_grammar.RESTRICTED, this.symbol)) {
  ...
  _createClass(Rule, [{
    ...
    value: function virtualize(expression) {
      ...
      var numeric = new Number(expression);
      ...
      if (comparison) {
        var _ret = function () {
          var op = comparison[1];
          var operands = expression.split(op, 1);
      ...
      if (arithmetic) {
        var _ret2 = function () {
          var op = arithmetic[1];
          var anchor = new RegExp('\\' + op + '(.+)?', 'g');
          var operands = expression.split(anchor, 2);
          var lhs = _this2.virtualize(operands[0]);
          var rhs = _this2.virtualize(operands[1]);
